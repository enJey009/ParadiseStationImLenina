...
/obj/machinery/r_n_d/circuit_imprinter/ussp
	name = "Платопринтер"
	desc = "Производит текстолитовые платы с золотыми контактами во славу СССП!"
	icon_state = "circuit_imprinter_ussp"
	icon_open = "circuit_imprinter_ussp_t"
	icon_closed = "circuit_imprinter_ussp"
	container_type = OPENCONTAINER
...
/obj/machinery/r_n_d/circuit_imprinter/ussp/New()
	..()
	component_parts = list()
	component_parts += new /obj/item/circuitboard/circuit_imprinter(null)
	component_parts += new /obj/item/stock_parts/matter_bin(null)
...
/obj/machinery/r_n_d/circuit_imprinter/ussp/upgraded/New()
	..()
	component_parts = list()
	component_parts += new /obj/item/circuitboard/circuit_imprinter(null)
	component_parts += new /obj/item/stock_parts/matter_bin/super(null)
...
/obj/machinery/r_n_d/circuit_imprinter/ussp/RefreshParts()
	reagents.maximum_volume = 0
	for(var/obj/item/reagent_containers/glass/G in component_parts)
		reagents.maximum_volume += G.volume
		G.reagents.trans_to(src, G.reagents.total_volume)
...
/obj/machinery/r_n_d/circuit_imprinter/ussp/check_mat(datum/design/being_built, var/M)
	var/list/all_materials = being_built.reagents_list + being_built.materials
...
/obj/machinery/r_n_d/circuit_imprinter/ussp/attackby(var/obj/item/O as obj, var/mob/user as mob, params)
	if(shocked)
		if(shock(user,50))
			return TRUE
	if(default_deconstruction_screwdriver(user, icon_open, icon_closed, O))
