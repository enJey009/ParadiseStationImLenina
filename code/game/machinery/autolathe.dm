...
/obj/machinery/autolathe
	name = "Автолат"
	desc = "Производит полезные предметы из металлов и стекла."
	icon_state = "autolathe_ussp"
	density = 1
	anchored = 1.0
	use_power = IDLE_POWER_USE
	idle_power_usage = 10
	active_power_usage = 100
	var/list/categories2 = list("Tools", "Electronics", "Construction", "Communication", "Security", "Machinery", "Medical", "Miscellaneous", "Dinnerware", "Imported")

/obj/machinery/autolathe/ussp/New()
	AddComponent(/datum/component/material_container, list(MAT_METAL, MAT_GLASS), _show_on_examine=TRUE, _after_insert=CALLBACK(src, .proc/AfterMaterialInsert_u))
	..()
	component_parts = list()
	component_parts += new /obj/item/circuitboard/autolathe(null)
	component_parts += new /obj/item/stock_parts/matter_bin(null)
	...
	wires = new(src)
	files = new /datum/research/autolathe/ussp(src)
	matching_designs = list()

/obj/machinery/autolathe/ussp/upgraded/New()
	..()
	component_parts = list()
	component_parts += new /obj/item/circuitboard/autolathe(null)
	component_parts += new /obj/item/stock_parts/matter_bin/super(null)
...
/obj/machinery/autolathe/ussp/Destroy()
	SStgui.close_uis(wires)
	QDEL_NULL(wires)
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
	materials.retrieve_all()
...
/obj/machinery/autolathe/ussp/interact(mob/user)
	if(shocked && !(stat & NOPOWER))
		if(shock(user, 50))
			return
...
/obj/machinery/autolathe/ussp/ui_interact(mob/user, ui_key = "main", datum/tgui/ui = null, force_open = TRUE, datum/tgui/master_ui = null, datum/ui_state/state = GLOB.default_state)
	ui = SStgui.try_update_ui(user, src, ui_key, ui, force_open)
	if(!ui)
		ui = new(user, src, ui_key, "Autolathe", name, 750, 700, master_ui, state)
		ui.open()
...
/obj/machinery/autolathe/ussp/ui_static_data(mob/user)
	var/list/data = list()
	...
	if(!recipiecache.len)
		...
		for(var/v in files.known_designs)
			var/datum/design/D = files.known_designs[v]
			var/list/cost_list = design_cost_data_u(D)
			var/list/matreq = list()
			for(var/list/x in cost_list)
				if(!x["amount"])
					continue
			...
			var/list/default_categories2 = D.category
			var/list/categories2 = istype(default_categories2) ? default_categories2.Copy() : list()

			if(imported[D.id])
				categories2 |= "Imported"
			...
			recipes.Add(list(list(
				...
				"uid" = D.UID(),
				"requirements" =  matreq,
				"hacked" = ("hacked" in categories2) ? TRUE : FALSE,
				"max_multiplier" = maxmult,
				"image" = "[icon2base64(icon(initial(I.icon), initial(I.icon_state), SOUTH, 1))]"
			)))
		recipiecache = recipes
...
/obj/machinery/autolathe/ussp/ui_data(mob/user)
	var/list/data = list()
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
	data["total_amount"] = materials.total_amount
	data["max_amount"] = materials.max_amount
...
/obj/machinery/autolathe/ussp/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	if(..())
		return FALSE
	...
	switch(action)
		if("clear_queue")
			queue = list()
		if("remove_from_queue_u")
			var/index = text2num(params["remove_from_queue_u"])
			if(isnum(index) && ISINRANGE(index, 1, queue.len))
				remove_from_queue_u(index)
				to_chat(usr, "<span class='notice'>Removed item from queue.</span>")
		if("make")
			BuildTurf = loc
			var/datum/design/design_last_ordered
			...
			if(!(design_last_ordered.id in files.known_designs))
				...
				return
			var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
			var/coeff = get_coeff_u(design_last_ordered)
			if(design_last_ordered.materials["$metal"] / coeff > materials.amount(MAT_METAL))
				to_chat(usr, "<span class='warning'>Invalid design (not enough metal)</span>")
				return
			if(design_last_ordered.materials["$glass"] / coeff > materials.amount(MAT_GLASS))
			...
			if(!(multiplier in list(1, 10, 25, max_multiplier))) //"enough materials ?" is checked in the build proc
				...
				return
			if((queue.len + 1) < queue_max_len)
				add_to_queue_u(design_last_ordered, multiplier)
			else
				to_chat(usr, "<span class='warning'>The autolathe queue is full!</span>")
			if(!busy)
				busy = TRUE
...
/obj/machinery/autolathe/ussp/ui_status(mob/user, datum/ui_state/state)
	. = disabled ? STATUS_DISABLED : STATUS_INTERACTIVE

	return min(..(), .)

/obj/machinery/autolathe/ussp/proc/design_cost_data_u(datum/design/D)
	var/list/data = list()
	var/coeff = get_coeff_u(D)
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
	var/has_metal = 1
	if(D.materials[MAT_METAL] && (materials.amount(MAT_METAL) < (D.materials[MAT_METAL] / coeff)))
		has_metal = 0
...
/obj/machinery/autolathe/ussp/proc/queue_data_u(list/data)
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
	var/temp_metal = materials.amount(MAT_METAL)
	var/temp_glass = materials.amount(MAT_GLASS)
	data["processing"] = being_built.len ? get_processing_line_u_u() : null
	if(istype(queue) && queue.len)
		var/list/data_queue = list()
		for(var/list/L in queue)
			var/datum/design/D = L[1]
			var/list/LL = get_design_cost_as_list_u(D, L[2])
			data_queue[++data_queue.len] = list("name" = initial(D.name), "can_build_u" = can_build_u(D, L[2], temp_metal, temp_glass), "multiplier" = L[2])
			temp_metal = max(temp_metal - LL[1], 1)
			temp_glass = max(temp_glass - LL[2], 1)
		data["queue"] = data_queue
		data["queue_len"] = data_queue.len
...
/obj/machinery/autolathe/ussp/attackby(obj/item/O, mob/user, params)
	if(busy)
		to_chat(user, "<span class='alert'>The autolathe is busy. Please wait for completion of previous operation.</span>")
		return 1
	if(exchange_parts(user, O))
...
/obj/machinery/autolathe/ussp/crowbar_act(mob/user, obj/item/I)
	if(!I.use_tool(src, user, 0, volume = 0))
		return
	. = TRUE
	if(busy)
...
/obj/machinery/autolathe/ussp/screwdriver_act(mob/user, obj/item/I)
	if(!I.use_tool(src, user, 0, volume = 0))
		return
	. = TRUE
	if(busy)
...
/obj/machinery/autolathe/ussp/wirecutter_act(mob/user, obj/item/I)
	if(!panel_open)
		return
	if(!I.use_tool(src, user, 0, volume = 0))
		return
...
/obj/machinery/autolathe/ussp/multitool_act(mob/user, obj/item/I)
	if(!panel_open)
		return
	if(!I.use_tool(src, user, 0, volume = 0))
		return
...
/obj/machinery/autolathe/ussp/proc/AfterMaterialInsert_u(type_inserted, id_inserted, amount_inserted)
	switch(id_inserted)
		if(MAT_METAL)
			flick("autolathe_o", src)//plays metal insertion animation
		if(MAT_GLASS)
...
/obj/machinery/autolathe/ussp/attack_ghost(mob/user)
	interact(user)

/obj/machinery/autolathe/ussp/attack_hand(mob/user)
	if(..(user, 0))
		return
	interact(user)

/obj/machinery/autolathe/ussp/RefreshParts()
	var/tot_rating = 0
	prod_coeff = 0
	for(var/obj/item/stock_parts/matter_bin/MB in component_parts)
		tot_rating += MB.rating
...
/obj/machinery/autolathe/ussp/proc/get_coeff_u(datum/design/D)
	var/coeff = (ispath(D.build_path,/obj/item/stack) ? 1 : 2 ** prod_coeff)//stacks are unaffected by production coefficient
	return coeff

/obj/machinery/autolathe/ussp/proc/build_item_u(datum/design/D, multiplier)
	desc = initial(desc)+"\nIt's building \a [initial(D.name)]."
	var/is_stack = ispath(D.build_path, /obj/item/stack)
	var/coeff = get_coeff_u(D)
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
	...
	var/glass_cost = D.materials[MAT_GLASS]
	var/power = max(2000, (metal_cost+glass_cost)*multiplier/5)
	if(can_build_u(D, multiplier))
		being_built = list(D, multiplier)
		use_power(power)
		icon_state = "autolathe_ussp"
		flick("autolathe_ussp_n",src)
		if(is_stack)
			var/list/materials_used = list(MAT_METAL=metal_cost*multiplier, MAT_GLASS=glass_cost*multiplier)
			materials.use_amount(materials_used)
		else
...
/obj/machinery/autolathe/ussp/proc/can_build_u(datum/design/D, multiplier = 1, custom_metal, custom_glass)
	if(D.make_reagents.len)
		return 0
...
/obj/machinery/autolathe/ussp/proc/get_design_cost_as_list_u(datum/design/D, multiplier = 1)
	var/list/OutputList = list(0,0)
	var/coeff = get_coeff_u(D)
	if(D.materials[MAT_METAL])
		OutputList[1] = (D.materials[MAT_METAL] / coeff)*multiplier
...
/obj/machinery/autolathe/ussp/proc/get_processing_line_u_u()
	var/datum/design/D = being_built[1]
	var/multiplier = being_built[2]
	var/is_stack = (multiplier>1)
	var/output = "PROCESSING: [initial(D.name)][is_stack?" (x[multiplier])":null]"
...
/obj/machinery/autolathe/ussp/proc/add_to_queue_u(D, multiplier)
	if(!istype(queue))
		queue = list()
	if(D)
		queue.Add(list(list(D,multiplier)))
...
/obj/machinery/autolathe/ussp/proc/remove_from_queue_u(index)
	if(!isnum(index) || !istype(queue) || (index<1 || index>queue.len))
		return 0
	queue.Cut(index,++index)
	return 1

/obj/machinery/autolathe/ussp/proc/process_queue_u()
	var/datum/design/D = queue[1][1]
	var/multiplier = queue[1][2]
	if(!D)
		remove_from_queue_u(1)
		if(queue.len)
			return process_queue_u()
		else
			return
	while(D)
		if((stat & (NOPOWER|BROKEN)) || disabled)
		...
		remove_from_queue_u(1)
		build_item_u(D,multiplier)
		D = listgetindex(listgetindex(queue, 1),1)
		multiplier = listgetindex(listgetindex(queue,1),2)
	being_built = new /list()

/obj/machinery/autolathe/ussp/proc/adjust_hacked_u(hack)
	hacked = hack
...
/obj/machinery/autolathe/ussp/proc/check_hacked_callback_u()
	if(!wires.is_cut(WIRE_AUTOLATHE_HACK))
		adjust_hacked_u(FALSE)

/obj/machinery/autolathe/ussp/proc/check_electrified_callback_u()
	if(!wires.is_cut(WIRE_ELECTRIFY))
		shocked = FALSE

/obj/machinery/autolathe/ussp/proc/check_disabled_callback_u()
	if(!wires.is_cut(WIRE_AUTOLATHE_DISABLE))
		disabled = FALSE

/obj/machinery/autolathe/ussp/security
	name = "Security Autolathe"
	desc = "Autolathe with preloaded open recipes"
	icon = 'icons/obj/sec_autolathe.dmi'

/obj/machinery/autolathe/ussp/security/New()
	. = ..()
	for (wires)
		wires.cut(WIRE_AUTOLATHE_HACK)
	adjust_hacked_u(TRUE)
