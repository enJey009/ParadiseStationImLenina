...
/obj/machinery/r_n_d/protolathe/ussp
	name = "Протолат"
	desc = "Создает полезные вещи из металла."
	icon_state = "protolathe_ussp"
	icon_open = "protolathe_ussp_t"
	icon_closed = "protolathe_ussp"
	container_type = OPENCONTAINER
...
/obj/machinery/r_n_d/protolathe/ussp/New()
	..()
	component_parts = list()
	component_parts += new /obj/item/circuitboard/protolathe(null)
	component_parts += new /obj/item/stock_parts/matter_bin(null)
...
/obj/machinery/r_n_d/protolathe/ussp/upgraded/New()
	..()
	component_parts = list()
	component_parts += new /obj/item/circuitboard/protolathe(null)
	component_parts += new /obj/item/stock_parts/matter_bin/super(null)
...
/obj/machinery/r_n_d/protolathe/ussp/RefreshParts()
	var/T = 0
	for(var/obj/item/reagent_containers/glass/G in component_parts)
		G.reagents.trans_to(src, G.reagents.total_volume)
	for(var/obj/item/stock_parts/matter_bin/M in component_parts)
...
/obj/machinery/r_n_d/protolathe/ussp/check_mat(datum/design/being_built, var/M)	// now returns how many times the item can be built with the material
	var/A = materials.amount(M)
	if(!A)
		A = reagents.get_reagent_amount(M)
		A = A / max(1, (being_built.reagents_list[M]))
...
/obj/machinery/r_n_d/protolathe/ussp/attackby(var/obj/item/O as obj, var/mob/user as mob, params)
	if(shocked)
		if(shock(user,50))
			return TRUE
	if(default_deconstruction_screwdriver(user, icon_open, icon_closed, O))
	...
	if(panel_open)
		if(istype(O, /obj/item/crowbar))
			...
			to_chat(user, "<span class='warning'>Вы не можете положить [src.name], пока панель открыта.</span>")
			return 1
	if(O.is_open_container())
		return FALSE
	else
